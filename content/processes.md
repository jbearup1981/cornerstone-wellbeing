...
Step summary:
- Discovery and qualification against the target profile
- Presentation
- Joint payroll-report build with client HR/payroll (register + W4 status)
- Quoting tool: AI census audit, qualification flags, discrepancy detection, eligibility cheat sheet, per-EE before/after paycheck
- Proposal generation, client review, signature
...
Step summary:
- Payroll setup: Section 125 deduction code + indemnity reimbursement code
- Initial payroll file build (upload-ready or manual entry format)
- Employee education window, 4 to 6 weeks, including Amaze comms and per-EE before/after statements
- Opt-out path routed through Amaze
- 100 percent audit of first payroll run
- Go-live

## Ongoing Service Process

Steady-state service runs on the client's payroll cycle — weekly or biweekly, whichever they're already on. Every cycle, Insight pulls the payroll register, either by the client emailing the report on a standing schedule or by Insight accessing the client's payroll system directly when that has been set up. The register goes into the Nexus quoting and audit system, which is the same engine that ran the initial census audit during sales.
...
- File ingested into Nexus eligibility audit system
- System flags still-qualified, no-longer-qualified, newly eligible, and reclassifications
